...
val stateSpaceIntro = <.div(
  ...
  marked("""
    ...
    |* [Heuristics](http://localhost:5173/ai4p/#/decks/heuristics/0), and what we as humans tend to do when the game starts getting big
    |* The search strategies that computers use when the state starts getting bigger, when they might need to find a solution but not necessarily the best one.
    |
